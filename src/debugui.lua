...
function debugui.create_handle(_x, _y, _w, _h)
	local el = {
		...
		drag = function(self)
			...
			if self.parent.x > Game.screen_width - self.parent.width then
				self.parent.x = Game.screen_width -
					self.parent.width
			end
			if self.parent.y < 0 then self.parent.y = 0 end
			if self.parent.y > Game.screen_height - self.parent.height then
				self.parent.y = Game.screen_height -
					self.parent.height
			end
		end,
		----------------
